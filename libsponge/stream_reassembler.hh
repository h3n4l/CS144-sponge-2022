 ...
 * @LastEditors: h3n4l
 * @LastEditTime: 2022-01-20 22:36:09
 * @FilePath: /CS144-sponge-2022/libsponge/stream_reassembler.hh
 */
...
class StreamReassembler {
   private:
    ...
    typedef struct node {
        ...
        uint64_t _index;
        uint64_t _end_index;
        node(std::string data, uint64_t index)
            : _data(data), _index(index), _end_index(index + data.size()) {}
    } node;
    std::list<node> _pad{};
