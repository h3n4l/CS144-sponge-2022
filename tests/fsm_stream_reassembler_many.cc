...
string read(StreamReassembler &reassembler) {
    return reassembler.stream_out().read(
...
int main() {
    try {
        ...
        // buffer a bunch of bytes, make sure we can empty and re-fill before
        // calling close()
        ...
        for (unsigned rep_no = 0; rep_no < NREPS; ++rep_no) {
            ...
            buf.push_substring(string(d.cbegin() + 7, d.cbegin() + 8), size + 7,
                               true);
        ...
        // insert EOF over previously queued data, require one of two possible
        // correct actions
        for (unsigned rep_no = 0; rep_no < NREPS; ++rep_no) {
            cerr << "[3] New REPS \n" << endl;
            StreamReassembler buf{65'000};
